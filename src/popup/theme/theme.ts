...
export const UI = {
    uiWrapper: "flex flex-col items-center justify-between py-8 h-full w-full bg-[var(--bg-primary)]",
    timerCircle: "w-[220px] h-[220px] bg-[var(--bg-timer)] rounded-full flex items-center justify-center flex-col shadow-2xl relative",
};

export function initTheme() {
    chrome.storage.local.get(['theme'], (result) => {
        const savedTheme:string =typeof result.theme === "string" ? result.theme : 'light';
